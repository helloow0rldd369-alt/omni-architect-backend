...
app.post('/auth/sync', async (req, res) => {
    try {
        ...
        const token = await (0, auth_1.verifySubscription)(key, uuid);
        const decoded = (0, auth_1.validateSystemJwt)(token);
...
wss.on('connection', (ws, req) => {
    ...
    try {
        const session = (0, auth_1.validateSystemJwt)(token);
        ws.on('message', async (messageData) => {
            ...
            if (payload.action === 'send') {
                ...
                const history = await (0, persistence_1.retrieveThreadHistory)(session.uid, threadId);
                ...
                await (0, persistence_1.saveTurnToThread)(session.uid, threadId, {
                    role: 'user',
                    content: userPrompt,
                    timestamp: Date.now()
                });
